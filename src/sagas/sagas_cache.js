...
function* updateCache() {
  try {
    ...
    const pageNumberRequestedInEndpoint =
      Math.ceil((activePageNumber + 1) / 8) - 1;
    ...
    const dataFetched = yield call(api.getNextCache, nextApiCallParams);
    ...
    const cacheToMerge = numbersOfPagesToMap.map(page => {
      ...
      const cardStartIndex =
        numbersOfPagesToMap.length === 4 ? (page - 4) * 12 : page * 12;
      const cardEndIndex =
        numbersOfPagesToMap.length === 4 ? (page - 3) * 12 : (page + 1) * 12;
...
function* fetchInitCache() {
  try {
    ...
    const totalCountOfCardsInEndpoint =
      initCacheResponse.headers["x-total-count"];
    const totalNumberOfPagesInEndpoint = Math.ceil(
      totalCountOfCardsInEndpoint / 12
    );
...
const cacheSagas = [fork(watchUpdateCache), fork(watchInitCache)];
